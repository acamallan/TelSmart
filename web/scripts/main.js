    ...
    if(result != ""){
        for (var i=0; i < result.length; i++){
            ...
            var value = result[i].split('^');
                ...
                display = display + "<td style='padding:4px;'>" + parseInt(value[3]).toFixed(2) + "</td>";
...
function tab5Paid(){
    ...
    if(GetValue("tab5_amount_paid") !== "" && parseInt(GetValue("tab5_amount_paid")) >= parseInt(GetValueInner("balance"))){
        ...
        var userid = getServerSession("userid");
        var transaction_id = GetCounter("001","10");
        var qryInsert = "insert into was_hotel_payment(invoice_number,transaction_date,transaction_time,customer_id,room_id,payment_mode,transaction_amount,userid,transaction_id)"
                + "values('"+invoice_number+"','"+transaction_date+"','"+transaction_time+"','"+customer_id+"','"+room_id+"','"+payment_mode+"','"+transaction_amount+"','"+userid+"','"+transaction_id+"')";
        var result = InsertDB(qryInsert);
        if(result === ""){
            HideLoading();
            return;
