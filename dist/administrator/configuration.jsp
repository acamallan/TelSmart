...
if(request.getParameter("save") != null){
    PreparedStatement qrySelect = conn.prepareStatement("select config_name, config_value from was_configuration where config_id='INVENTORY'");
    ResultSet strResult = qrySelect.executeQuery();
    String set = "";
    Integer strReturn = 0;
    while(strResult.next()){
        set =  "config_value = '"+request.getParameter(strResult.getString("config_name"))+"' where config_name = '"+strResult.getString("config_name")+"' and config_id='INVENTORY'";
    ...
    if(strReturn.equals(1)){
        PreparedStatement qryConfig = conn.prepareStatement("select config_name, config_value, config_desc from was_configuration where config_id='INVENTORY'");
...
PreparedStatement qrySelect = conn.prepareStatement("select config_name, config_value, config_desc from was_configuration where config_id='INVENTORY'");
ResultSet strResult = qrySelect.executeQuery();
%>
<fieldset style="width:700px; height: 150px; margin: 0 auto; margin-top: 30px; border: 1px solid #ddd; background-color: #f6f6f6; padding:5px;"  >
    <legend style="font-size: 12px; font-weight: bold; padding:0 10px">Configuration</legend>
    <form action="configuration.jsp" method="post" autocomplete="off">
    <div style="position: relative;">
        ...
        <div class="block" style="margin-left: 50px; top: <%= top %>px;">
            <label style="width:200px"><%= strResult.getString("config_desc") %></label>
